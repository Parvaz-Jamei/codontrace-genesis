...
class SignalActionLink:
    """Temporal link from signal/memory events to later action/reward.

    Evidence ladder (scientific, claim-strict):
    - observed_write: a memory write was recorded
    - temporal_correlation: write later co-occurs with reward (not causal)
    - read_linked: explicit memory_read precedes the decision/reward
    - causal_support: requires paired ablation/control digest (engine cannot invent this)

    ``correct_delayed_action`` is true only for runtime-flagged or read-linked
    success paths. Temporal correlation alone never sets it. Claim eligibility
    remains false unless causal_support evidence is present.
    """
    ...
    evidence_status: str = "not_classified"
    causal_status: str = "correlational_only"
    control_digest: str | None = None
    claim_eligible: bool = False
    schema_version: str = "signal_action_link_v2"
...
class MemoryUseEvidence(SignalActionLink):
    schema_version: str = "memory_use_evidence_v2"


@dataclass(frozen=True, slots=True)
class DelayedRewardTrace(SignalActionLink):
    schema_version: str = "delayed_reward_trace_v2"
