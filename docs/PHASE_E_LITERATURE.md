...
| Goldsby et al. coordination instructions / division of labor | Messaging buffer with `send_message`, `retrieve_message`, `broadcast_message`, `block_propagation`; DoL *metrics* (`build_deme_division_of_labor_observation`); `communication_ablation_status=not_run` | Not evolved communication, language, or literature-grade DoL |
