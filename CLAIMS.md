...
| Sexual recombination substrate | Explicit `ReproductionMode.SEXUAL_CROSSOVER` / `life_loop_world(reproduction_mode=...)` builds a recombinant child from two viable parents via a deterministic positional segment swap, then mutation, and records both parent ids on lineage / birth evidence. Research defaults remain asexual. | Allowed as software capability / runtime observation. Do not claim sexual selection, evolved instinct, intelligence, or Avida-replacement status. |
