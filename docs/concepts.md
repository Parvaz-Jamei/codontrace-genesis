...
`ObstacleConfig.block_movement` and `block_sight` are also preserved metadata in `v0.3.0a1`. They do not change default movement or sensing behavior yet. Default movement still treats `World2D.walls` as blocking cells, and there is no line-of-sight physics or raycasting.
