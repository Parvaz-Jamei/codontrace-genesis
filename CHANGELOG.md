...
- The English BAIC file is rebuilt in the congress template, not an IEEE
  ...
  that the ESP32 adapter is an engineering stub with no robot result. Figure 1
  in the English manuscript is the path from a domain profile to the grade,
  with the risk bar drawn apart from that grade. The English reference list
  now cites Avida, MABE 2.0, the ODD protocol, Holm's correction, and the
  two software archives that the text names: 0.3.0b6 and 0.3.0a2.
