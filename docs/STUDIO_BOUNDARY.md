...
Status: **required for `0.3.0b3` beta and later**.
...
Studio work belongs in a separate consumer repository, for example `codontrace-studio`, which may depend on `codontrace==0.3.0b3` and adapt the public Python object API into local REST/WebSocket/UI flows.
