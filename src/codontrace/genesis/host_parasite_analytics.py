...
SCHEMA_FRONT_DOOR = "host_parasite_front_door_mediation_v1"


@dataclass(frozen=True, slots=True)
class FrontDoorCellResult:
    """One cell in the front-door observational product design."""

    cell_id: str
    role: str  # "x0" | "x1" | "m_probe" | "dual_null"
    schedule_arm: str
    ablation_preset: str
    ...
    def __post_init__(self) -> None:
        cid = _refuse_banned_fragment(_as_str(self.cell_id, "cell_id"), "cell_id")
        object.__setattr__(self, "cell_id", cid)
        role = _as_str(self.role, "role").casefold()
        if role not in {"x0", "x1", "m_probe", "dual_null"}:
            raise ConfigurationError("role must be x0, x1, m_probe, or dual_null.")
        ...
        object.__setattr__(
            self,
            "mediator_value",
            float(require_finite_float("mediator_value", self.mediator_value)),
        )
        object.__setattr__(
            self,
            "outcome_value",
            float(require_finite_float("outcome_value", self.outcome_value)),
        )
        object.__setattr__(self, "meter_digest", _as_str(self.meter_digest, "meter_digest"))
        object.__setattr__(self, "world_digest", _as_str(self.world_digest, "world_digest"))
        computed = canonical_digest(
            {
                ...
                "schedule_arm": self.schedule_arm,
                "ablation_preset": self.ablation_preset,
                "mediator_value": self.mediator_value,
        ...
        object.__setattr__(
            self, "cell_digest", _check_digest(self.cell_digest, computed, "FrontDoorCellResult")
        )
...
@dataclass(frozen=True, slots=True)
class FrontDoorMediationContrast:
    """Front-door style observational product; observational_bounds only."""

    contrast_id: str
    cells: tuple[FrontDoorCellResult, ...]
    front_door_estimate: float
    effect_lower: float
    effect_upper: float
    dual_null_delta: float
    mediator_channel: str
    outcome_channel: str
    identifiability: str = "observational_bounds"
    claim_role: str = "exploratory"
    claim_ceiling: str = "runtime_observation"
    ...
    digest: str = ""
    intervention_supported: bool = False
    front_door_identified: bool = False
    red_queen_proved: bool = False
    raises_claim_ladder: bool = False
    ...
    def __post_init__(self) -> None:
        ...
        object.__setattr__(self, "contrast_id", cid)
        if len(self.cells) < 3:
            raise ConfigurationError("front-door requires at least 3 cells.")
        object.__setattr__(self, "cells", tuple(self.cells))
        object.__setattr__(
            self,
            "front_door_estimate",
            float(require_finite_float("front_door_estimate", self.front_door_estimate)),
        )
        object.__setattr__(
            self,
            "effect_lower",
        ...
        object.__setattr__(
            ...
            "mediator_channel",
            _refuse_banned_fragment(
                _as_str(self.mediator_channel, "mediator_channel"), "mediator_channel"
            ),
        )
        object.__setattr__(
            self,
            "outcome_channel",
            _refuse_banned_fragment(
                _as_str(self.outcome_channel, "outcome_channel"), "outcome_channel"
            ),
        )
        ident = _as_str(self.identifiability, "identifiability")
        if ident != "observational_bounds":
        ...
        if self.intervention_supported:
            raise ConfigurationError("refuses intervention_supported=True.")
        if self.front_door_identified:
            raise ConfigurationError("refuses front_door_identified=True.")
        if self.red_queen_proved:
            raise ConfigurationError("refuses red_queen_proved=True.")
        if self.raises_claim_ladder:
            raise ConfigurationError("refuses raises_claim_ladder=True.")
        object.__setattr__(self, "intervention_supported", False)
        object.__setattr__(self, "front_door_identified", False)
        object.__setattr__(self, "red_queen_proved", False)
        object.__setattr__(self, "raises_claim_ladder", False)
        computed = canonical_digest(self._body(), prefix="hp_fd_ctr")
        object.__setattr__(
            self,
            "digest",
            _check_digest(self.digest, computed, "FrontDoorMediationContrast"),
        )

    def _body(self) -> dict[str, JsonValue]:
        return {
            "schema": SCHEMA_FRONT_DOOR,
            "contrast_id": self.contrast_id,
            "cells": [c.to_dict() for c in self.cells],
            "front_door_estimate": self.front_door_estimate,
            "effect_lower": self.effect_lower,
            "effect_upper": self.effect_upper,
            "dual_null_delta": self.dual_null_delta,
            "mediator_channel": self.mediator_channel,
            "outcome_channel": self.outcome_channel,
            "identifiability": self.identifiability,
            "claim_role": self.claim_role,
            "claim_ceiling": self.claim_ceiling,
            "smoke_only": self.smoke_only,
...
def run_front_door_mediation_contrast(
    *,
    contrast_id: str,
    seed: int = 0,
    ticks: int = 2,
    treat_schedule_arm: str = "freeze",
    control_schedule_arm: str = "none",
    mediator_channel: str = "match_pass",
    outcome_channel: str = "coupling_total",
    claim_role: str = "exploratory",
    smoke_only: bool = True,
    claim_ceiling: str = "runtime_observation",
) -> FrontDoorMediationContrast:
    """Observational front-door product proxy on ScheduleLock × meter channels.

    Design (refuse-safe, not nonparametric ID):
      x0: control schedule → (M0, Y0)
      x1: treat schedule → (M1, Y1)
      m_probe: treat schedule + mild ablation on non-mediator path
      dual_null: honesty cell
    estimate ≈ (M1 - M0) * ((Y1 - Y0) / max(|M1 - M0|, eps))  [scaled path product]
    """

    med_ch = _refuse_banned_fragment(
        _as_str(mediator_channel, "mediator_channel"), "mediator_channel"
    )
    out_ch = _refuse_banned_fragment(
        _as_str(outcome_channel, "outcome_channel"), "outcome_channel"
    )
    specs: list[tuple[str, str, str, str]] = [
        ("x0", "x0", control_schedule_arm, "none"),
        ("x1", "x1", treat_schedule_arm, "none"),
        ("m_probe", "m_probe", treat_schedule_arm, "structure_null"),
        ("dual_null", "dual_null", "none", "dual_null"),
    ]
    cells: list[FrontDoorCellResult] = []
    values_m: dict[str, float] = {}
    values_y: dict[str, float] = {}
    for cell_id, role, sched, abl in specs:
        abl_use = abl
        raw = _run_world_cell(
            profile_id=f"fd_{contrast_id}_{cell_id}"[:48],
            seed=seed,
            ticks=ticks,
            coupling_amount=0.25,
            spatial_mode="well_mixed",
            ...
            match_score_scale=1.0,
            schedule_arm=sched,
            ablation_preset=abl_use,
        )
        mv = _channel_value(raw, med_ch)
        yv = _channel_value(raw, out_ch)
        values_m[cell_id] = mv
        values_y[cell_id] = yv
        cells.append(
            FrontDoorCellResult(
    ...
    dm = values_m["x1"] - values_m["x0"]
    dy = values_y["x1"] - values_y["x0"]
    # path-product proxy; if mediator does not move, estimate collapses toward 0
    scale = abs(dm) if abs(dm) > 1e-12 else 1.0
    estimate = dm * (dy / scale)
    # alternative honesty: m_probe should attenuate mediator path
    dual_delta = values_y["dual_null"] - values_y["x0"]
    half = abs(estimate)
    return FrontDoorMediationContrast(
        contrast_id=contrast_id,
        cells=tuple(cells),
        front_door_estimate=estimate,
        effect_lower=estimate - half,
        effect_upper=estimate + half,
        dual_null_delta=dual_delta,
        mediator_channel=med_ch,
        outcome_channel=out_ch,
        claim_role=claim_role,
        claim_ceiling=claim_ceiling,
        smoke_only=smoke_only,
    )
