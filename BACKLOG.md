...
16. Hard experiment 01 Wave 1 (implemented, preregistered causal design): four paired arms including `capsules_shuffled` specificity control, `min_source_fitness` dose ladder, explicit DAG + `do()` interventions, paired `d_z` / BCa / sign-flip / Holm. Library default 12 seeds = exploratory; research-grade n=30. Claim ceiling stays `runtime_observation` until a dated digest-backed campaign is archived. Not a Phase M. Wave 2/3 (ClaimGate extract, publish) stay out of scope.
