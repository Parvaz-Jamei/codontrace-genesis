...
```bash
python -m pip install codontrace==0.3.0b3
```

For research extras:

```bash
python -m pip install "codontrace[research]==0.3.0b3"
```

For causal-analysis extras:

```bash
python -m pip install "codontrace[causal]==0.3.0b3"
```

For quality-diversity extras:

```bash
python -m pip install "codontrace[qd]==0.3.0b3"
```
...
Use `codontrace==0.3.0b3` for public-beta reproducibility work.

Older development outputs may be useful as development evidence, but should not be presented as release evidence for `0.3.0b3` unless they are rerun on the public release.
...
> Public-beta reproducibility should target `codontrace==0.3.0b3` and DOI `10.5281/zenodo.20337435`.
