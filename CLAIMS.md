...
Package: `codontrace==0.3.0b3`
Development identity on `main`: `0.3.0b4.dev0` (unreleased; Phases H–L + HARD_EXPERIMENT_01). Published PyPI tip: `0.3.0b3` (A–G).
...
| Packaging | CodonTrace Genesis is published as a Python package and can be installed from PyPI as `codontrace==0.3.0b3`. | Tie install claims to the exact version. |
...
| Named materials / chemistry-effect substrate (Phase G) | Explicit `GenesisRuntimeProfile.materials_world()` / `life_loop_world(materials=...)` runs named `MaterialSpec` pools and optional spatial grids with energy-yield, toxicity, viscosity/diffusion, permeability, signaling, and scarcity coefficients; eat/absorb → ATP/merit; toxins → ATP drain; catalysts enable a stoichiometric CRN subset; membrane permeability gates uptake; named-material chemostat (Phase C inflow/outflow); digest-backed trajectory replay; `external_ontology_id` / units / effect_coefficients schema hooks for a later ChEBI/KEGG/BiGG table. Optional autocatalytic cycle is a measurement demo. Defaults off so A–E digests stay stable. | Allowed as software capability / runtime observation. Do not claim realistic chemistry, wet-lab equivalence, KEGG/BiGG GEM solving, molecular dynamics, evolved chemotons, spatial ACE, intelligence, or Avida-replacement. |
...
It should **not** be used as a publication-grade claim until the run is repeated on the current public release (`0.3.0b3` or newer), archived with its configuration/artifacts, and reported with enough seeds and controls.
...
- The inspected output was generated from an older `0.2.0a25` development/release artifact, not the current public `0.3.0b3` PyPI release.
- The inspected summary includes a release-identity/provenance warning around the tested zip name.
- Therefore this snapshot is valid as **development evidence** and **instrumentation evidence**, but not as a final public benchmark for `0.3.0b3`.
...
(Wave 1b, after survival calibration).
...
v1 ClaimGate ceiling: **`runtime_observation`** (level 1). v1 is an
**invalid null** (every arm extinct, adoptions 0; source-bias gate never
acted). Wave 1b adds an assay gate and overlay calibration; CLAIMS.md
records only the ceiling ClaimGate actually grants on v2. This is not
mechanism support and not collective intelligence.
...
>
> **HARD_EXPERIMENT_01 Wave 1 (30-seed research) is a runtime observation of an invalid-null paired delta: populations crashed and the source-bias gate was never exercised. ClaimGate did not grant `intervention_supported`. Wave 1b keeps that ceiling unless ClaimGate grants a higher existing label on the calibrated v2 artifact.**
...
- HARD_EXPERIMENT_01 Wave 1 recorded a 30-seed research campaign whose ClaimGate ceiling is `runtime_observation` only (identical last-tick fitness across four arms; invalid null / assay failure). Wave 1b does not loosen ClaimGate.
...
The following claims must not be made for `0.3.0b3` unless a future paper/benchmark explicitly satisfies the required evidence level.
...
- a benchmark is rerun on `0.3.0b3` or newer,
