...
| Live `devosoft/avida` `avida.cfg` `VERSION_ID` 2.14.0 config groups | Authoritative knob names for reproduction, sex, demes, messaging, energy, sensing, resources | See [§2 Avida.cfg group map](#2-avidacfg-2140-group-map) | mixed: **landed / partial / deferred** per group |
...
| Empirical MODES / systematics integration notes | Persistence filter is sensitive to coalescence window; full phylogenetic systematics + shadow runs are the careful path | Documented coalescence-window semantics; `empirical_systematics_shadow_run=False` | **partial** docs+tests; **deferred** full Empirical systematics shadow run |
...
| Goldsby messaging / GermlineReplication / demes wiki | `send_message`, `retrieve_message`, `broadcast_message`, `block_propagation`; soma vs germline | Phase E deme messaging + role gates | **partial** |
| GECCO 2008 digital germlines | Cooperative networks / germline replication | Role / propagule-eligibility tags | **partial** |
| Am Nat 2020 associative learning | Learning in Avida | Capsule/memory slots with **real** subsequent action/ATP/task effects | **partial** (hooks + runtime effect, not proved learning) |
| PLOS One odometry case study | Navigation / odometry in digital organisms | Same capsule/memory mapping | **partial** (hooks vs proved learning) |
...
| Logic-9 / reaction–task metabolic coupling | Tasks as reactions consuming resources | **deferred** | — |
...
| Sexual recombination / birth chamber | `RECOMBINATION_GROUP` | n/a | n/a | n/a | diploid meiosis deferred | **landed** opt-in; diploid **deferred** |
...
| Empirical systematics shadow phylogeny | n/a | **deferred** | shadow hook recorded as unavailable unless supplied | n/a | n/a | **deferred** (`empirical_systematics_shadow_run=False`) |
...
- `no_empirical_systematics_shadow_run` unless a caller supplies a real shadow digest
...
- [x] Phase D MODES persistence filter documents coalescence-window semantics and exposes `persistence_window_t`; no silent claim of a full Empirical systematics shadow run.
...
- [x] Multi-seed required for anything above measurement_only; pass claim still blocked.
...
- [x] No intelligence / AGI / OEE-proved / Avida-replacement language.
...
- Persistence filtering is not a full phylogenetic systematics proof.
- Capsule/memory effects are not proved associative learning.
- Deme messaging is not collective intelligence.
