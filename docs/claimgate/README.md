...
| `test_snapshot.json` | 27 pytest cases green in one sandbox run | GitHub Actions matrix / multi-OS proof |

Rebuild validation:

```bash
PYTHONPATH=src python tools/export_ladder_validation.py
PYTHONPATH=src python -m pytest tests/test_claimgate_ladder_packs.py tests/test_claimgate_biomedical.py tests/test_claimgate_adapters.py
```
